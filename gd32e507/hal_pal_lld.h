 ...
 */
#define PAL_PORT(line)    ((uint32_t)((uint32_t)(line) & 0xFFFFFFF0U))                                                

/**
 * @brief   Decodes a pad identifier from a line identifier.
 */
#define PAL_PAD(line)                                                       \
  (1 << ((uint32_t)((uint32_t)(line) & 0x0000000FU)))
 ...
 * @notapi
 */
#define pal_lld_readpad(port, pad) (GPIO_ISTAT(port) & pad ? 0x01 : 0x00)
...
#define pal_lld_setpadmode(port, pad, mode)                                  \
  gpio_init(port, (mode >> 8) & 0xFF, mode & 0xFF,pad); 
